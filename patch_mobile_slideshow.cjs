...
code = code.replace(
  ...
  `className="relative overflow-hidden rounded-2xl bg-slate-950 text-white min-h-[280px] sm:min-h-[320px] border border-slate-200/20 shadow-md flex flex-col justify-between transition-all duration-500"`
);

code = code.replace(
  `className="absolute inset-0 bg-cover bg-center bg-no-repeat transition-all duration-700 transform scale-102"`,
