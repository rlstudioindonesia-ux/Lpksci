...
const listRegex = /\{\/\* Bagian Kelas Reguler \*\/\}[\s\S]*?\{\/\* Bagian Kelas Alumni \*\/\}[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*\)\}\s*<\/div>\s*\{selectedClassForChapters/m;
...
const replacement = `                  
                  ...
                  <div className="space-y-3 mt-8">
                    <div className="flex items-center gap-2 border-b border-emerald-100 pb-2">
                      ...
                      <h5 className="font-bold text-slate-800 text-sm">Kelas Alumni</h5>
                      <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full text-[10px] font-bold">
                        {allClasses.filter(c => c.type === "alumni").length} Kelas
                      </span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {allClasses.filter(c => c.type === "alumni").map((c) => {
                        ...
                        const isExpanded = expandedClassIds.includes(c.id);
                        return (
                          <div key={c.id} className="bg-white rounded-2xl p-4 border border-emerald-100 flex flex-col gap-3 relative overflow-hidden group shadow-sm hover:shadow-md transition">
                            {/* Indicator */}
                            ...
                            <button 
                               onClick={() => toggleExpand(c.id)} 
                               className="ml-2 mt-1 w-full text-left flex items-center justify-between p-2 rounded-xl bg-emerald-50/50 hover:bg-emerald-50 border border-emerald-100 transition cursor-pointer text-emerald-700 font-semibold text-xs"
                            >
                               <span>{isExpanded ? "Sembunyikan Detail" : "Lihat Detail"}</span>
                               <ChevronDown className={\`w-4 h-4 transition-transform \${isExpanded ? "rotate-180" : ""}\`} />
                            </button>
...
console.log("Patch AdminKelasSegment Expand done!");
